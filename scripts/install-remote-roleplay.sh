...
if [[ ! -d "${staging_root}/site" ]] ||
  [[ ! -f "${staging_root}/site/runtime-config.js" ]] ||
  [[ ! -f "${staging_root}/compose.yaml" ]]; then
...
if [[ ! "${roleplay_revision}" =~ ^[0-9a-f]{40}$ ]]; then
  echo "Invalid Rusty Roleplay revision: ${roleplay_revision}" >&2
  exit 1
fi
...
"${compose[@]}" up --detach --force-recreate --wait rusty-roleplay
...
if [[ "${index_html}" != *'<app-root'* ]] || [[ "${index_html}" != *'<title>roleplay-web</title>'* ]]; then
  echo "Rusty Roleplay HTML verification failed at ${site_url}" >&2
  exit 1
fi

database_path="${deployment_root}/data/engine/coordination.sqlite3"
if [[ ! -f "${database_path}" ]]; then
  echo "Rusty Roleplay SQLite database was not created at ${database_path}" >&2
  exit 1
fi
